...
public class LFACostOptF {
	...
	public LFACostOptF(DirectedSparseMultigraph<Node, Link> g, int T, int T2)
	{
	...
	//			}
				for (int j = 1; j < T+1; j++) {
					...
					if(lfaFTempGMain.lfaEfficiency<100){
						...
						{
							tempGLinkIT.elementAt(randomNum).cost = originalCost - j;
							tempG.findEdge(tempGLinkIT.elementAt(randomNum).tailEnd, tempGLinkIT.elementAt(randomNum).headEnd).cost = originalCost - j;
							//Dijkstra algorithm calculation
							dijkstra();
							//LFA
